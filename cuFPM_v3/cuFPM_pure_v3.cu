...
void mexFunction(
    ...
    mxGPUArray * wavefront1  = mxGPUCopyFromMxArray(prhs[0]); // sample wavefront
    mxGPUArray * wavefront2  = mxGPUCopyFromMxArray(prhs[1]); // pupil wavefront
    const mxGPUArray * obseY  = mxGPUCreateFromMxArray(prhs[2]); // observed intensity
    const mxGPUArray * ledIdx = mxGPUCreateFromMxArray(prhs[3]); // LED position in pixel
    const mxGPUArray * pupil  = mxGPUCreateFromMxArray(prhs[4]);
    ...
    creal32_t * __restrict__ d_wavefront1 = (creal32_t * __restrict__ ) mxGPUGetData(wavefront1);
    creal32_t * __restrict__ d_wavefront2 = (creal32_t * __restrict__ ) mxGPUGetData(wavefront2);

    const real32_T * __restrict__ d_obseY = (const real32_T * __restrict__ ) mxGPUGetDataReadOnly(obseY);
    const real32_T * __restrict__ d_pupil = (const real32_T * __restrict__) mxGPUGetDataReadOnly(pupil);   
    const int2 * __restrict__ d_ledIdx    = (const int2 * __restrict__ ) mxGPUGetDataReadOnly(ledIdx);
    ...
    cudaMemset(mom_w1, 0, (imHs_sz.x * imHs_sz.y) * sizeof(float));
    cudaMemset(mom_w2, 0, (imLs_sz.x * imLs_sz.y) * sizeof(float));
    ...
    int inembed[2] = {(int) imLs_sz.x, (int) imLs_sz.y};
  
    imLs_sz.z = batch_sz;

    cufftHandle cuFFtplanLarge;
    cufftHandle cuFFtplanBatch;
    cufftPlan2d(&cuFFtplanLarge, imHs_sz.x, imHs_sz.y, CUFFT_C2C);
    cufftPlanMany(
        &cuFFtplanBatch, 2, 
        &inembed[0], &inembed[0], // inembed
        1, (int) imLs_sz.x * (int) imLs_sz.y, // idist
        &inembed[0], // inembed
        1, (int) imLs_sz.x * (int) imLs_sz.y, // idist
        CUFFT_C2C, (int) imLs_sz.z
    );
    ...
    for(int epoch = 0; epoch < numEpochs; ++ epoch){
        ...
        while(remain > 0){
            ...
            cufftExecC2C(
                ...
                (cufftComplex *)&d_dldw1[0],
                CUFFT_FORWARD
            );
            N_BLOCKS.z = (unsigned) min(curr + batch_sz - 1, (led_total-1) ) - curr + 1;
            
            fused_getSubpupil_shifted<<<N_BLOCKS, N_THREADS>>>(
                curr,
                d_dldw1, d_wavefront2,
                d_ledIdx,
                imLs_sz, imHs_sz,
                subwave, latentZ
            );
            ...
            cufftExecC2C(
                cuFFtplanBatch, 
                (cufftComplex *)&latentZ[0], 
                (cufftComplex *)&latentZ[0], 
                CUFFT_INVERSE
            );

            differenceKernel_acc<<<N_BLOCKS, N_THREADS>>>(
                curr,
                imLs_sz, pratio,
                d_obseY,
                latentZ, recordZ
            ...
            cufftExecC2C(
                cuFFtplanBatch, 
                (cufftComplex *)&recordZ[0], 
                (cufftComplex *)&latentZ[0], 
                CUFFT_FORWARD
            );
            cudaMemset(d_dldw1, 0, (imHs_sz.x * imHs_sz.y) * sizeof(creal32_t));
            cudaMemset(d_dldw2, 0, (imLs_sz.x * imLs_sz.y) * sizeof(creal32_t));
            ...
            constrained_RMSprop_both<<<N_BLOCKS_L, N_THREADS>>>(
                d_dldw1, d_dldw2,
                beta,
                learning_rate,
                eps,
    ...
    cudaFree(mom_w2);
    // free cufft handle
    cufftDestroy(cuFFtplanBatch);
    cufftDestroy(cuFFtplanLarge);
    // free mxGPUArray
