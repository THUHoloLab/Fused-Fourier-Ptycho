...
__global__ void getSubpupil(
    ...
    if (inside) {
        unsigned page_id = c_begin + idz * (imgSz.x * imgSz.y);
...
__global__ void setConstraint(
    ...
    float ratio = (float) (imLs_sz.x * imLs_sz.y * pratio * pratio);
    float2 lat_A = {latentz[pix_A + page_id].re / ratio, 
                    latentz[pix_A + page_id].im / ratio};
                    
    float y_AXY = yObs[pix_A + page_id];
    float tempA = sqrtf(lat_A.x * lat_A.x + lat_A.y * lat_A.y);

    float2 lat_X;

    lat_X = {latentz[pix_X + page_id].re / ratio, 
             latentz[pix_X + page_id].im / ratio};

    blk_x[tr_x + 1][tr_y] = (sqrtf(lat_X.x *lat_X.x + lat_X.y * lat_X.y) 
                                    - yObs[pix_X + page_id]) - (tempA - y_AXY);

    lat_X = {latentz[pix_Y + page_id].re / ratio, 
             latentz[pix_Y + page_id].im / ratio};

    blk_y[tr_x][tr_y + 1] = (sqrtf(lat_X.x *lat_X.x + lat_X.y * lat_X.y) 
                                    - yObs[pix_Y + page_id]) - (tempA - y_AXY);
    block.sync();

    unsigned test_p2 = 0;
    if (tr_x == 0){
        test_p2 = (idx == 0)? ((imLs_sz.x - 1) * imLs_sz.y + idy) : ((idx - 1) * imLs_sz.y + idy);
        lat_X = {latentz[test_p2 + page_id].re / ratio, 
                 latentz[test_p2 + page_id].im / ratio};
        blk_x[0][tr_y] = (tempA - y_AXY) - 
                         (sqrtf(lat_X.x *lat_X.x + lat_X.y * lat_X.y) - yObs[test_p2 + page_id]);
    }
    ...
    if (tr_y == 0){
        ...
        lat_X = {latentz[test_p2 + page_id].re / ratio, 
                 latentz[test_p2 + page_id].im / ratio};
        blk_y[tr_x][0] = (tempA - y_AXY) - 
                         (sqrtf(lat_X.x *lat_X.x + lat_X.y * lat_X.y) - yObs[test_p2 + page_id]);
    }

    blk_x[tr_x][tr_y] = copysignf(1.0f,blk_x[tr_x][tr_y]);
    blk_y[tr_x][tr_y] = copysignf(1.0f,blk_y[tr_x][tr_y]);
    block.sync();

    if (tr_x == 0){
        blk_x[BLOCK_X][tr_y] = copysignf(1.0f,blk_x[BLOCK_X][tr_y]);
    }
    if (tr_y == 0){    
        blk_y[tr_x][BLOCK_Y] = copysignf(1.0f,blk_y[tr_x][BLOCK_Y]);
    }    
    ...
    float ang = atan2f(lat_A.y,lat_A.x);

    if (inside){
        latentz[pix_A + page_id].re = cosf(ang) * tempA;
        latentz[pix_A + page_id].im = sinf(ang) * tempA;
    }
}